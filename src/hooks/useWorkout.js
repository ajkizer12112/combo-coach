...
const useWorkout = () => {
    ...
    const timer = {
        ...
        decrementTimer: function () {
            if (shouldShowNextCombo(workout)) return comboFns.activateNextCombo();
            if (shouldHideCombo(workout)) return comboFns.hideCombo();
        ...
        runZero: function () {
            if (shouldFinishWorkout(workout)) workoutFns.completeWorkout();
            else if (workout.currentPhase === REST) workoutFns.changeRound();
            else if (workout.currentPhase === COUNTDOWN) workoutFns.endCountdown();
            else if (workout.currentPhase === WORK) workoutFns.startRest();
            else return
        },
        runWarningChecks: function () {
            if (shouldPlayWorkWarning(workout)) {
                sounds.playWarning();
            }

            if (shouldPlayRestWarning(workout)) {
                sounds.playWarning();
            }

            if (shouldPlayRoundChangeWarning(workout)) {
    ...
    return { workout, timer, workoutFns }
}
